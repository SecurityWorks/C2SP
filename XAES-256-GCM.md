...
AES-256-GCM counter-intuitively has less security margin given the same number
of rounds because of AES's poor key schedule. Since we are deriving keys, we
could decide to rip out the AES key schedule and just derive round subkeys,
resolving the issue and improving the cipher. That would obviously be
non-standard, and would require us to use a hash-based key derivation.
